// Test with smaller system to debug SIGFPE
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a cubic crystal of Argon (3^3 = 27 atoms for easier debugging)
    for i in 0..3 {
        for j in 0..3 {
            for k in 0..3 {
    ...
    let trajectory_out = Box::new(TrajectoryOutput::new("trajectory_small.xyz")?);
    simulation.add_output_with_frequency(trajectory_out, 10);

    let energy_out = Box::new(EnergyOutput::new("energy_small.dat")?);
